...
async def remove_events(user, id, token, session):
    try:
        #logging.info(user)
        async with session.delete(
            f"https://graph.microsoft.com/v1.0/users/{user}/events/{id}",
            headers={
                'Authorization': 'Bearer ' + token['access_token']
        }) as response_delete:

            if response_delete.status == 204:
                logging.info(f"Event {id} deleted.")
            ...
            elif response_delete.status == 404:
                logging.warning(f"Event {id} not found {response_delete.status} {response_delete.reason}.")
                #return None
            else:
                logging.error(f"Some errors getting event {id} {response_delete.status} {response_delete.reason}")
...
async def process_users_page(users_data, token, session):
    ...
    if "value" in users_data:
        for user in users_data["value"]:
            tasks = []
            task = asyncio.ensure_future(remove_events(user, token, session))
            tasks.append(task)        
        ...
        await remove_events(users_data, token, session)
        #teams_not_activated2 = [team for team in teams_not_activated if team]
        #teams_not_activated.append(await get_team(group['id'], token, session))
...
async def get_data(token, session):
    ...
    with open(csv_file_read, mode='r') as csv_file:
        ...
        for row in csv_reader:
            ...
            task = asyncio.ensure_future(remove_events(row["userPrincipalName"], row["Eventid"], token, session))
            tasks.append(task)
