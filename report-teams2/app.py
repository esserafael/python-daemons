...
async def get_events(user, token, session):
    async with session.get(
        f"https://graph.microsoft.com/v1.0/users/{user['id']}/calendarView/?endDateTime=2021-03-06T00:00:00Z&startDateTime=2021-03-01T01:00:00Z",
        headers={
...
async def get_data(token, session):
    ...
    csv_file_read = os.path.join(os.path.dirname(__file__), "aulas_01_03.csv")
        ...
        for row in csv_reader:
            ...
            if task_counter <= 50:
                ...
                task = asyncio.ensure_future(
                    get_events(
                        await get_users(f"https://graph.microsoft.com/v1.0/users/{row['EMAIL']}", token, session),
                        #await get_users(row['EMAIL'], token, session),
                        token, session
                        )
                    )
                ...
                logging.info("Sending 50 tasks(users).")
